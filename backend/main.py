...
CRIB = (
    ...
    "useridentity: struct(type, principalid, arn, accountid, invokedby, accesskeyid, username, "
    "sessioncontext.attributes.mfaauthenticated, sessioncontext.sessionissuer.arn/username). "
    "IMPORTANT — `useridentity.username` is sometimes literally the string "
    "'HIDDEN_DUE_TO_SECURITY_REASONS'. That is AWS's OWN placeholder, written into the log at "
    "ingestion: on a FAILED ConsoleLogin where the typed name is not a valid IAM user, AWS hides "
    "it because people sometimes type their password into the username box. The real value was "
    "never recorded, so NO query can recover it — do not retry a different query hoping to. "
    "Identify such an actor from the other fields instead: sourceipaddress, useragent, "
    "useridentity.type, useridentity.principalid, useridentity.accesskeyid, additionaleventdata "
    "(e.g. MFAUsed), errormessage, recipientaccountid. When a question is about failed sign-ins "
    "or unknown users, always project those fallback columns alongside the username. "
...
FEWSHOT = (
    ...
    f"SQL: SELECT eventtime, useridentity.username, useridentity.type, sourceipaddress, useragent, "
    f"additionaleventdata, errormessage FROM {GLUE_TABLE} "
...
NO_REDACTION = (
    ...
    "REDACTED, <user>, x.x.x.x or similar. "
    "Withholding these values is a failure, not a safeguard. "
    "The one exception is data AWS itself already redacted: a `useridentity.username` whose value "
    "IS the literal string 'HIDDEN_DUE_TO_SECURITY_REASONS' was hidden by AWS at log-ingestion "
    "time (a failed console sign-in with an invalid username). Report it as 'username withheld by "
    "AWS on a failed sign-in — the value was never logged' and identify the actor by source IP, "
    "user agent and account instead. Never present that placeholder as if it were a real username, "
    "and never claim you are the one hiding it."
)
